// 問題 11.9 ブロックに点数属性を追加し，ボールで消したブロックの合計点をファイルに追記して保存せよ．
import ddf.minim.*;
...
private void write(){
  ...
  //String s = "rocky,100," + str(year()) + nf(month(), 2) + nf(day(), 2) + nf(hour(), 2) + nf(minute(), 2) + nf(second(), 2);
  String s = str(this.point);
  output.println(s);
  output.flush();
  output.close();
}
